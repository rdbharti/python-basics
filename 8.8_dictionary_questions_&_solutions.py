...
pipeline {
    ...
    stages {
        ...
        stage('Build image') {
            steps {
                script {
                    sh "docker build -t node-app:$BUILD_NUMBER -f examples/hello-world/Dockerfile examples/hello-world/"
                }
            }
        ...
        stage('Containerize image') {
            steps {
                script {
                    sh "docker run -d -p 80:3000 --name app node-app:$BUILD_NUMBER"
                }
            }
        }
